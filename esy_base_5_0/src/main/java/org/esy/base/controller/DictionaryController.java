 ...
 */
@Controller
@RequestMapping("/api/base/dictionary")
public class DictionaryController {
	...
	@ResponseBody
	public ResponseEntity<Response> insert(@RequestBody Dictionary o, HttpServletRequest request) {
		...
		try {
			o.setUid(null);
			resp = new Response(0, "Save success.", dictionaryService.save(o, request));
	...
	@ResponseBody
	public ResponseEntity<Response> update(@PathVariable String uid, @RequestBody Dictionary o,
		...
		Dictionary obj = dictionaryService.getByUid(uid);
		if (obj != null) {
			if (obj.getUid().equals(o.getUid())) {
				resp = new Response(0, "Update success.", dictionaryService.save(o, request));
				return new ResponseEntity<Response>(resp, HttpStatus.OK);
			} else {
	...
	public ResponseEntity<Response> query(@RequestParam Map<String, Object> parm, HttpServletRequest request,
		...
		// 检查权限
		// result = dictionaryService.checkSearch(parm);
		// if (result.getBody().getError() != 0) {
		// return result;
		// }

		Response resp;
		resp = new Response(0, "success.", dictionaryService.query(parm));
		return new ResponseEntity<Response>(resp, HttpStatus.OK);
	}
