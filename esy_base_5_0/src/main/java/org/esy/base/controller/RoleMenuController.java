...
@Controller
@RequestMapping("/api/base/rolemenu")
public class RoleMenuController {
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		RoleMenu o = roleMenuDao.getByUid(uid);
		if (o == null) {
	...
	@ResponseBody
	public ResponseEntity<Response> addDetail(@RequestBody RoleMenu o, HttpServletRequest request) {
		...
		Response resp;
		try {
			resp = new Response(0, "Save success.", roleMenuService.save(o));
			return new ResponseEntity<Response>(resp, HttpStatus.OK);
		} catch (Exception e) {
			e.printStackTrace();
			e.printStackTrace();
	...
	@ResponseBody
	public ResponseEntity<Response> update(@PathVariable String uid, @RequestBody RoleMenu o,
		...
		obj = roleMenuDao.getByUid(uid);
		if (obj != null) {
			if (obj.getUid().equals(o.getUid())) {
				resp = new Response(0, "Save success.", roleMenuDao.save(o));
				return new ResponseEntity<Response>(resp, HttpStatus.OK);
			} else {
	...
	public ResponseEntity<Response> query(@RequestParam Map<String, Object> parm, HttpServletRequest request,
		...
		resp = new Response(0, "success.", roleMenuDao.query(parm));
		return new ResponseEntity<Response>(resp, HttpStatus.OK);
