...
@Controller
@RequestMapping("/api/base/mqueuecfg")
public class MQueueCfgController {

	public static final String AUTHORITY = "base_config_mq";
	
	@Autowired
	private ILoginService loginService;

	@Autowired
	private IMQueueCfgDao iMQueueCfgDao;
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		MQueueCfg o = iMQueueCfgDao.getByUid(uid);
		if (o == null) {
	...
	@ResponseBody
	public ResponseEntity<Response> insert(@RequestBody MQueueCfg o, HttpServletRequest request) {
		...
		try {
			o.setUid(null);
			resp = new Response(0, "Save success.", iMQueueCfgDao.save(o));
			return new ResponseEntity<Response>(resp, HttpStatus.OK);
		} catch (Exception e) {
	...
	@ResponseBody
	public ResponseEntity<Response> update(@PathVariable String uid, @RequestBody MQueueCfg o,
		...
		obj = iMQueueCfgDao.getByUid(uid);
		if (obj != null) {
	...
	public ResponseEntity<Response> query(@RequestParam Map<String, Object> parm, HttpServletRequest request,
		...
		resp = new Response(0, "success.", iMQueueCfgDao.query(parm));
		return new ResponseEntity<Response>(resp, HttpStatus.OK);
