...
@Controller
@RequestMapping("/api/base/logmodule")
public class LogModuleController {

	public static final String AUTHORITY = "base_config_event";
	
	@Autowired
	private ILoginService loginService;

	@Autowired
	private ILogModuleDao logModuleDao;
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		LogModule o = logModuleDao.getByUid(uid);
		if (o == null) {
	...
	@ResponseBody
	public ResponseEntity<Response> insert(@RequestBody LogModule o, HttpServletRequest request) {
		...
		try {
			o.setUid(null);
			resp = new Response(0, "Save success.", logModuleDao.save(o));
			return new ResponseEntity<Response>(resp, HttpStatus.OK);
		} catch (Exception e) {
	...
	@ResponseBody
	public ResponseEntity<Response> update(@PathVariable String uid, @RequestBody LogModule o,
	...
	public ResponseEntity<Response> query(@RequestParam Map<String, Object> parm, HttpServletRequest request,
		...
		resp = new Response(0, "success.", logModuleDao.query(parm));
		return new ResponseEntity<Response>(resp, HttpStatus.OK);
