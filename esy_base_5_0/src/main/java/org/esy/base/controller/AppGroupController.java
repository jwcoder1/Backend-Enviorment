 ...
 */
@Controller
@RequestMapping("/api/base/appGroup")
public class AppGroupController {
	...
	@ResponseBody
	public ResponseEntity<Response> add(@RequestBody AppGroup o, HttpServletRequest request) {
		...
		try {
			o.setUid(null);
			resp = new Response(0, "Save success.", appGroupService.save(o, null, request));
	...
	public ResponseEntity<Response> delet(@PathVariable String uid, HttpServletRequest request) {
		...
		AppGroup o = appGroupService.getByUid(uid);
		if (o == null) {
			resp = new Response(HttpStatus.GONE.value(), "Object not found", null);
			return new ResponseEntity<Response>(resp, HttpStatus.GONE);
		} else {
			if (YESUtil.objtolong(appGroupService.getChildrenCount(o.getAgid())) > 0) {
				resp = new Response(HttpStatus.INTERNAL_SERVER_ERROR.value(), "该应用分组下有子分组,不可删除", null);
				return new ResponseEntity<Response>(resp, HttpStatus.INTERNAL_SERVER_ERROR);
			}
			try {
				appGroupService.delete(o);
	...
	@ResponseBody
	public ResponseEntity<Response> update(@PathVariable String uid, @RequestBody AppGroup o, HttpServletRequest request) {
		ResponseEntity<Response> result = RestUtils.checkAuthorization(request, loginService, AUTHORITY);
		if (result.getBody().getError() != 0) {
			return result;
		}
		...
		Response resp;
		AppGroup obj = appGroupService.getByUid(uid);
		if (obj != null) {
			if (obj.getUid().equals(o.getUid())) {
				resp = new Response(0, "Update success.", appGroupService.save(o, obj, request));
				return new ResponseEntity<Response>(resp, HttpStatus.OK);
			} else {
	...
	public ResponseEntity<Response> query(@RequestParam Map<String, Object> parm, HttpServletRequest request, HttpServletResponse response) {
		...
		Response resp;
		resp = new Response(0, "success.", appGroupService.query(parm));
		return new ResponseEntity<Response>(resp, HttpStatus.OK);
	...
	public ResponseEntity<Response> changeLocation(@RequestBody Map<String, Object> parm, HttpServletRequest request, HttpServletResponse response) {
		...
		MsgResult mr = appGroupService.changeLocation(parm, "AppGroup");
		if (!mr.getSuccess()) {
